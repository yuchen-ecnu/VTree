...
int main(int argc, char* argv[]){
	...
	*/
	//K = 10;
        vector<int> o;
        int node=Nodes.size();
        char filename[]="bj_real_0";
        FILE* file;
        double cset[4]={0.001,0.01,0.05,0.07};
        for(int m=0;m<4;m++){
        double cc=cset[m];
	...
	for(int m=0;m<1;m++){
	int K=10;
	for(int i=0;i<node*0.001;i++)
		o.push_back(rand()%node);
	vector<int> re;
	for(int i=0;i<1000;i++){
		int S=rand()%node;
		knn_query(S,K,o,re);
	...
	o3.clear();
	o4.clear();
	fscanf(fin,"%d %d",&S,&size);
	//long sum1,sum2,sum3,sum4;long sum1,sum2,sum3,sum4;
	...
	vector<int> result;
	TIME_TICK_START
		knn_query(S,10,o1,result);
	TIME_TICK_END
		sum1+=(te-ts);
	TIME_TICK_START
		knn_query(S,10,o2,result);
	TIME_TICK_END
		sum2+=(te-ts);
		TIME_TICK_START
		knn_query(S,10,o3,result);
	TIME_TICK_END
		sum3+=(te-ts);
		TIME_TICK_START
		knn_query(S,10,o4,result);
	TIME_TICK_END
		sum4+=(te-ts);
	}
	cout<<"TIME="<<endl;
