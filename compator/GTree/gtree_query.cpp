...
struct timeval tv;
long long ts, te;
void TIME_TICK_START() {gettimeofday(&tv,NULL); ts = tv.tv_sec * 100000 + tv.tv_usec / 10;}
void TIME_TICK_END() {gettimeofday(&tv,NULL); te = tv.tv_sec * 100000 + tv.tv_usec / 10;}
void TIME_TICK_PRINT(const char *T) {printf("%s RESULT: %lld (0.01MS)\r\n", (T), te - ts );}
// ----------
