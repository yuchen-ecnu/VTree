...
struct Graph//Struct of the graph
{
    ...
    vector<int> Split(Graph *G[],
                      int nparts)//Partition the graph to two parts, and stores to G1, G2,2 METIS algorithm,npart is the number of parts
    {
...
struct V_Tree {
    ...
    struct Interface {
        ...
        friend istream &operator>>(istream &in, Interface &I) {
            ...
            I.node[0].min_car_dist.emplace_back(0, 0);
            return in;
        }
        ...
        void read_binary(istream &in) {
            ...
            node[0].min_car_dist.emplace_back(0, 0);
        }
    ...
    //calculate the id of border in father/son
    {
        ...
        for (x = 1; x < node.tot; x++) {
            ...
            for (int j = 0; j < node[x].borders.size(); j++) {
                ...
                node[x].min_car_dist.emplace_back(INF, -1);
            }
...
//    }
    ...
    bool push_borders_up_del_min_car_dist(int x,
                                          int start_id)//del  min_car_dist which node_id=start_id from, then update. no exist return false, otherwise return true
    {
...
int main(int argc, char *argv[]) {
    ...
    float car_per = std::atof(argv[4]);
    float change_per = std::atof(argv[5]);
    float query_number = std::atof(argv[6]);
    fflush(stdout);
    TIME_TICK_START();
    init();
